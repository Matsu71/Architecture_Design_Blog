import {KANTO,ERAS,VISITS,escapeHtml as e,hasCoordinates,filterBuildings,directionsUrl,photoHtml} from './core.mjs';
import {nearbyScreenPoints,locationLabel} from './location.mjs';
const $=id=>document.getElementById(id);
let all=[],byId=new Map(),results=[],map=null,active=null,markers=new Map(),visibleLabels=false,pickerOrigin=null,highlighted=null,cards=new Map();
...
function select(id,{pan=false,scroll=false,commit=true}={}){
  if(commit)active=id;
  if(highlighted!==id){
    for(const key of new Set([highlighted,id])){
      if(!key)continue;
      const isActive=key===id,marker=markers.get(key);
      marker?.getElement()?.classList.toggle('active',isActive);
      marker?.setZIndexOffset(isActive?1000:0);
      cards.get(key)?.classList.toggle('is-active',isActive);
    }
    highlighted=id;
  }
  if(scroll)cards.get(id)?.scrollIntoView({block:'nearest',behavior:reduceMotion?'instant':'smooth'});
  const b=byId.get(id);
...
function showOne(b){
  ...
  const marker=markers.get(b.id);if(marker)marker.bindPopup(`<div class="map-popup"><strong>${e(b.nameJa)}</strong><span>${e(b.address)}</span><small>${e(locationLabel(b.location))}</small><a href="${e(articleUrl(b))}">建物を読む →</a><a href="${e(directionsUrl(b))}" target="_blank" rel="noopener noreferrer">行き方</a></div>`).openPopup();
}
...
function apply({fit=true}={}){
  ...
  closePicker();renderList();renderMarkers();highlighted=active;
  ...
  const advanced=['architect','type','era','visit'].filter(key=>f[key]).length+(f.sort&&f.sort!=='name'?1:0);
  $('more-filters').textContent=advanced?`絞り込み ${advanced}`:'絞り込み';
...
async function boot(){
  ...
  try{
    ...
    if(['architect','type','era','visit','sort'].some(k=>initial.has(k))){$('extra-filters').hidden=false;$('more-filters').setAttribute('aria-expanded','true');}
    ...
    if(!map)document.querySelectorAll('.view-switch button:not([data-view="list"])').forEach(button=>{button.disabled=true;});
    let timer,composing=false;
    const requestFilter=()=>{clearTimeout(timer);timer=setTimeout(()=>apply(),180);};
    controls.query.addEventListener('compositionstart',()=>{composing=true;clearTimeout(timer);});
    controls.query.addEventListener('compositionend',()=>{composing=false;requestFilter();});
    Object.entries(controls).forEach(([k,c])=>c.addEventListener(k==='query'?'input':'change',event=>{
      clearTimeout(timer);
      if(k==='query'){if(!composing&&!event.isComposing)requestFilter();}
      else apply();
    }));
    $('filters').onsubmit=event=>{event.preventDefault();if(composing)return;clearTimeout(timer);apply();};
    $('reset-filters').onclick=()=>{clearTimeout(timer);composing=false;reset();};
