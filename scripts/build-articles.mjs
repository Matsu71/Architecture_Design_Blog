...
const fields=['id','slug','nameJa','nameEn','aliases','prefecture','municipality','area','address','lat','lng','completionYear','era','architects','buildingTypes','styles','materials','oneLiner'];
const index=data.map(b=>({...Object.fromEntries(fields.map(k=>[k,b[k]])),location:{status:b.location.status,precision:b.location.precision},visit:{status:b.visit.status},...(b.photo?{photo:b.photo}:{})}));
await output('data/map-index.json',JSON.stringify(index,null,2)+'\n');
const geography={type:'FeatureCollection',license:'ODbL 1.0',licenseUrl:'https://opendatacommons.org/licenses/odbl/1-0/',attribution:'OpenStreetMap contributors',features:data.filter(b=>b.location?.evidence?.kind==='building-outline').map(b=>({type:'Feature',id:b.id,properties:{osmWayId:b.location.evidence.osmWayId,sourceTimestamp:b.location.evidence.sourceTimestamp,point:[b.lng,b.lat],sourceUrl:`https://www.openstreetmap.org/way/${b.location.evidence.osmWayId}`},geometry:{type:'Polygon',coordinates:[b.location.evidence.ring]}}))};
await output('data/building-footprints.geojson',JSON.stringify(geography,null,2)+'\n');
